...
public class Applicator3Test {
    ...
    public void calculateFile() throws IOException {
        OrbitConfig conf = new OrbitConfig(40, 4, 0, true, 3);
    ...
    private static long flip(long fbs, int fromIndex, int toIndex) {
    ...
    private static long orShift(long fbs, long other, int v, int sh) {
        if (sh == 0) {
            return fbs | other;
        } else {
            int cut = v - sh;
            long end = other >>> cut;
            long beg = other & (WORD_MASK >>> (BITS_PER_WORD - cut));
            return fbs | end | (beg << sh);
        }
    }

    private static long diffShift(long fbs, long other, int v, int sh) {
        if (sh == 0) {
            return fbs & ~other;
        } else {
            int cut = v - sh;
