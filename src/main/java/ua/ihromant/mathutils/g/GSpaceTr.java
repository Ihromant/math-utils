...
public class GSpaceTr {
    ...
    private final StateTr[][][] statesCache;

    private final int[][] auths;

    public GSpaceTr(int k, Group group, boolean genAuths, int[][] comps) {
    ...
    public GSpaceTr(int k, Group table, boolean genAuths, SubGroup... subs) {
        this.group = table;
        this.k = k;
        ...
        QuickFind qf = new QuickFind(v * v * v);
            ...
            for (int g = 0; g < gOrd; g++) {
                ...
                for (int x2 = 0; x2 < v; x2++) {
                    ...
                    for (int x3 = 0; x3 < v; x3++) {
                        int gx3 = cayley[g][x3];
                        qf.union(to(x1, x2, x3), to(gx1, gx2, gx3));
                    }
                }
        ...
        this.emptyFilter = new FixBS(v * v * v);
        for (int i = 0; i < v; i++) {
            for (int j = 0; j < v; j++) {
                emptyFilter.set(to(i, i, j));
                emptyFilter.set(to(i, j, i));
                emptyFilter.set(to(j, i, i));
            }
        }
        this.differences = qf.components().stream().filter(c -> !c.intersects(emptyFilter)).toArray(FixBS[]::new);
        this.diffMap = new int[v * v * v];
        ...
        this.statesCache = new StateTr[oBeg.length][v][v];
            ...
            for (int snd = fst + 1; snd < v; snd++) {
                for (int trd = snd + 1; trd < v; trd++) {
                    statesCache[oi][snd][trd] = Objects.requireNonNull(
                            new StateTr(FixBS.of(v, fst), FixBS.of(gOrd, 0), new FixBS(sz), new int[sz][], 1)
    ...
    public int to(int p1, int p2, int p3) {
        return p1 * v * v + p2 * v + p3;
    }
    ...
    public StateTr forInitial(int fst, int snd, int trd) {
    ...
    public boolean minimal(StateTr[] states) {
        ex: for (int[] auth : auths) {
    ...
    public boolean parMinimal(StateTr[] states) {
        return Arrays.stream(auths).parallel().allMatch(auth -> {
            FixBS[] altBlocks = new FixBS[states.length];
            for (int i = 0; i < states.length; i++) {
                FixBS block = states[i].block();
