...
public class FourFieldPoint {
    ...
    private static BitSet[] generateLines() {
        ...
        for (int i = 0; i < FIELD.length; i++) {
            ...
            line.set(FIELD.length * FIELD.length + FIELD.length); // 20
            lines[lineIdx] = line;
        }
    ...
    public static String lineToString(int i) {
        return line(i).stream().mapToObj(FourFieldPoint::pointToString).collect(Collectors.joining(",", "[", "]"));
    }

    public static String lineToString(int i, BitSet filter) {
        return line(i).stream().filter(p -> !filter.get(p)).mapToObj(FourFieldPoint::pointToString).collect(Collectors.joining(",", "[", "]"));
    }
}
